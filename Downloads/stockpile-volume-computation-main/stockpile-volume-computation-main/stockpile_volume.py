...
=============================
Computes the volume of a stockpile from a 3D point cloud (.ply file)
using Open3D for point cloud processing and Delaunay triangulation.

Based on: https://jose-llorens-ripolles.medium.com/stockpile-volume-with-open3d-fa9d32099b6f

Pipeline:
    1. Load point cloud from PLY file
    2. Segment ground plane using RANSAC
    3. Separate stockpile points from ground
    4. Align point cloud (translate + rotate so ground = Z=0)
    5. Remove statistical outliers
    6. Voxel downsample for efficiency
    7. Delaunay triangulation on XY projection
    8. Build triangle mesh surface
    9. Compute volume under each triangle prism
   10. Output total stockpile volume
